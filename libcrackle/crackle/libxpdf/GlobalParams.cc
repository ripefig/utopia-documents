...
GlobalParams::GlobalParams(const char *cfgFileName) {
  ...
  if (f) {
#if 0 // don't parse standard xpdf options in utopia
...
void GlobalParams::setupBaseFonts(char *dir) {
  ...
  GBool verbose=0;
  const char *show_fonts=getenv("PDF_SHOW_FONTS");
  if(show_fonts && strcmp(show_fonts,"0")!=0) {
    verbose=1;
  }
  ...
  for (i = 0; displayFontTab[i].name; ++i) {
    ...
    if(verbose) {
      std::cerr << "Looking for " << fontName->getCString() << std::endl;
    }

    if (dir) {
      fileName = appendToPath(new GString(dir), displayFontTab[i].t1FileName);
      if(verbose) {
        std::cerr << "  ... trying " << fileName->getCString() << std::endl;
      }
    ...
    if (!fileName) {
      for (j = 0; !fileName && displayFontDirs[j]; ++j) {
        if(displayFontTab[i].ttFileName) {
          fileName = appendToPath(new GString(displayFontDirs[j]), displayFontTab[i].ttFileName);
          if(verbose) {
            std::cerr << "  ... trying " << fileName->getCString() << std::endl;
          }
          if ((f = fopen(fileName->getCString(), "rb"))) {
        ...
        if(!fileName && displayFontTab[i].t1FileName) {
          fileName = appendToPath(new GString(displayFontDirs[j]), displayFontTab[i].t1FileName);
          if(verbose) {
            std::cerr << "  ... trying " << fileName->getCString() << std::endl;
          }
          if ((f = fopen(fileName->getCString(), "rb"))) {
    ...
    if (!fileName) {
      if(verbose) {
        std::cerr << "WARNING could not find font for " << fontName->getCString() << std::endl;
      }
      delete fontName;
      continue;
    } else {
      if(verbose) {
        std::cerr << "USING " << fileName->getCString() << " for " << fontName->getCString() << std::endl;
      }
    }
